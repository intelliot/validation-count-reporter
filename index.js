...
function saveValidation(validation) {
  ...
  if (!validation.full) {
    ...
    messageSlack(':x: *partial validation* from `' + validation.validation_public_key + '` for `' + validation.ledger_hash + '`')
  }
  ...
  for (let index in ledgers) {
    if (smoment().diff(ledgers[index].timestamp) > 10000) {
      ...
      for (let hash in ledgers[index].hashes) {
        messageSlack(':x: `' + index + '` `' + hash + '` received :' + ((ledgers[index].hashes[hash].length < numbers.length) ? numbers[ledgers[index].hashes[hash].length] : ledgers[index].hashes[hash].length) + ': validations')
      }
