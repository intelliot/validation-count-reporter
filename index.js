  ...
  getChannelList.then((res) => {
    ...
    if (channel) {
      ...
      rtm
        ...
        .then((x) => {
          console.log(
            `Message sent to channel ${channel.name} - ${JSON.stringify(
              x,
              null,
              2,
            )}`,
...
const valListUrl = process.env['testnet']
...
function saveManifest(manifest) {
  ...
  if (validators[pubkey] && validators[pubkey].seq < manifest.seq) {
    ...
    messageSlack(
      '<!channel> :scroll: new manifest for: `' +
        getName(pubkey) +
        '`: #' +
        validators[pubkey].seq +
        ', `' +
        validators[pubkey].signing_key +
        '`',
    )
  }
}
...
function saveValidation(validation) {
  validation.validation_public_key =
    manifestKeys[validation.validation_public_key]
  ...
  const rows = []
  const key = [validation.ledger_hash, validation.validation_public_key].join(
    '|',
  )
  ...
  if (!validation.full) {
    let partialValidationMessage =
      '#' +
      validation.ledger_hash.slice(0, 6) +
      ' `' +
      validation.ledger_index +
      '` *partial validation* from `' +
      getName(validation.validation_public_key) +
      '`'
    trouble[validation.validation_public_key] = partialValidationMessage
    let trouble_keys = Object.keys(trouble)
    if (trouble_keys.length === 2) {
      // at least 2 validators having problems
      partialValidationMessage = trouble_keys.reduce(
  ...
  ledgers[validation.ledger_index].hashes[validation.ledger_hash].push(
    validation.validation_public_key,
  )
  if (
    ledgers[validation.ledger_index].hashes[validation.ledger_hash].length ==
    ...
    if (validationCount !== Object.keys(validators).length) {
      console.log(
        `WARNING: validationCount changed from ${validationCount} to ${
          Object.keys(validators).length
        }`,
      )
      reportValidations()
      const emoji =
        validationCount < Object.keys(validators).length
          ...
          : ':warning:'
      messageSlack(
        `${emoji} previous ledgers received ${validationCount} validations, but the next ledger (\`${
          validation.ledger_index
        }\` \`${validation.ledger_hash}\`) received *${
          Object.keys(validators).length
        }* validations!`,
      )
      validationCount = Object.keys(validators).length
    ...
    if (highestLedgerIndexToReport === undefined) {
      ...
      parseInt(highestLedgerIndexToReport) + 1 ===
      parseInt(validation.ledger_index)
    ) {
      // OK - subsequent good ledger of this reporting cycle
    } else {
      const highestSeenLedger = highestLedgerIndexToReport
      console.log(
        `WARNING: we skipped from ledger ${highestSeenLedger} to ${validation.ledger_index}`,
      )
      reportValidations()
...
function reportValidations(type) {
  ...
  const secondsPerLedger = (
    moment().diff(lastReportTimestamp) /
    1000 /
    ledgerCount
  ).toFixed(3)
  ...
  const report = `${emoji} ledgers \`${lowestLedgerIndexToReport}\` to \`${highestLedgerIndexToReport}\` ${flagLedgerNote}all received *${validationCount}* validations. That's ${ledgerCount} ledgers ${moment().from(
    lastReportTimestamp,
  )}, or ${secondsPerLedger} seconds per ledger.`
  messageSlack(report)
...
function subscribeToRippleds() {
  ...
  resolve(
    process.env['testnet'] ? 'r.altnet.rippletest.net' : 'r.ripple.com',
  ).then((ips) => {
    console.log(ips)
    for (const ip of ips) {
    ...
    if (moment().diff(ledgers[index].timestamp) > 10000) {
      ...
      for (let hash in ledgers[index].hashes) {
        ...
        if (ledgerWasLessThan5SecAfterStartup) {
          message += `\n:information_desk_person: Since I saw this ledger ${moment
            .duration(ledgers[index].timestamp.diff(startTimestamp))
            .asSeconds()
            .toFixed(
              3,
            )} seconds after starting up, there may be other validations that I did not see: `
        } else {
          // message += '\n:x: `';
        }
        ...
        message +=
          ...
          index +
          '` received ' +
          ledgers[index].hashes[hash].length +
          ' validations'
        if (
          ledgers[index].hashes[hash].length <
          Object.keys(validators).length / 2
        ) {
          message += ' from'
          for (var i = 0; i < ledgers[index].hashes[hash].length; i++) {
            message += ' `' + getName(ledgers[index].hashes[hash][i]) + '`,'
...
function setName(pubkey) {
  request
    ...
    .then((data) => {
      if (data.domain) names[pubkey] = data.domain
    })
}
      ...
      for (const validator of valList.validators) {
        ...
        if (!validators[pubkey] || validators[pubkey].seq < manifest.Sequence) {
          if (validators[pubkey]) {
            ...
            validators[pubkey] = {}
            if (getName(pubkey) === pubkey) setName(pubkey)
            if (!startup)
              messageSlack(
                '<!channel> :tada: new trusted validator: `' +
                  getName(pubkey) +
                  '`',
              )
          }
